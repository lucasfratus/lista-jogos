...
'''
    ...
    tam_headLed = int.from_bytes(entrada.read(2))
    entrada.seek(1, os.SEEK_CUR) # Pula o '*'
    tam_reg = len(registro) + 2 # Soma o tamanho do registro mais um espaço de dois bytes em que ficará seu offset 
    headLed_ant = entrada.read(4) # Guarda o byteoffset da próxima cabeça menor que a atual e maior que as outras
    entrada.seek(int.from_bytes(headLed_ant), os.SEEK_SET)
    tam_headLed_ant = int.from_bytes(entrada.read(2))
    if tam_headLed >= tam_reg:
        tam_sobra = tam_headLed - tam_reg
        entrada.seek(headLed + tam_sobra, os.SEEK_SET)
        entrada.write((len(registro)).to_bytes(2))
        registro_bytes = registro.encode()
        entrada.write(registro_bytes)
        entrada.seek(headLed, os.SEEK_SET)
        if tam_sobra >= 10: # Caso essa condição ocorra, o espaço que sobra volta pra LED
            if tam_sobra >= tam_headLed_ant:
                entrada.seek(os.SEEK_SET)
                entrada.write(tam_sobra.to_bytes(4))
            else: # tam_sobra < tam_headLed_ant 
                entrada.seek(int.from_bytes(headLed_ant)+2, os.SEEK_SET)
                while tam_headLed_ant > tam_sobra:
                    headLed_ant1 = headLed_ant # Guarda o registro que ocupava a cabeça da LED anteriormente ao atual
    
                    entrada.seek(1, os.SEEK_CUR) # Pula o '*'
                    headLed_ant = int.from_bytes(entrada.read(4)) # Lê o byteoffset do próximo removido (menor que o anterior)
                    #headLed_ant = int.from_bytes(headLed) # Converte o byteoffset para inteiro
                    entrada.seek(headLed_ant, os.SEEK_SET) # Posiciona o ponteiro na posição do registro indicado pela cabeça da LED
                    tam_headLed = int.from_bytes(entrada.read(2)) # Guarda o tamanho do registro indicado pela cabeça da LED
                entrada.seek(os.SEEK_SET)
                entrada.write(int.from_bytes(headLed_ant1))
                entrada.seek(headLed_ant1+3, os.SEEK_SET) # Posiciona o ponteiro após o '*' do menor registro maior que o último removido
                entrada.write(headLed.to_bytes(4)) # Escreve no menor registro maior que o último removido o byteoffset do último removido
                entrada.seek(headLed+3, os.SEEK_SET) # Posiciona o ponteiro após o '*' do último registro removido
                entrada.write(headLed_ant.to_bytes(4))  # Escreve após o '*' do último registro removido o próximo espaço disponível menor que ele (se não houver, escreve -1)
        else: #tam_sobra < 10:
            entrada.seek(1, os.SEEK_CUR)
            entrada.write('\0\0\0'.encode())
    else: # O tamanho do novo registro não cabe na cabeça da LED, portanto deve ser inserido no final do arquivo
        entrada.seek(0, os.SEEK_END)
        entrada.write((len(registro)).to_bytes(2))
        registro_bytes = registro.encode()
